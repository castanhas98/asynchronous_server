#ifndef ASYNCHRONOUS_SERVER_THREADSAFE_DATA_STRUCTURES_HPP
#define ASYNCHRONOUS_SERVER_THREADSAFE_DATA_STRUCTURES_HPP
...
public:
  ...
  // : buckets_(number_buckets, std::make_unique<ThreadsafeBucketPair<Key,Value>>()),
  : hasher_(hasher) { 
    for(int i = 0; i < number_buckets; ++i)
      buckets_.push_back(
        std::unique_ptr<ThreadsafeBucketPair<Key, Value>>(
          new ThreadsafeBucketPair<Key, Value>()
        )
      );
  }
...
private:
  const ThreadsafeBucketPair<Key, Value>& get_bucket(const Key& key) const {
    typename std::vector<ThreadsafeBucketPair<Key, Value>>::size_type idx;
    idx = hasher_(key) % buckets_.size();

    return *buckets_[idx];
  }

  ThreadsafeBucketPair<Key, Value>& get_bucket(const Key& key) {
    typename std::vector<ThreadsafeBucketPair<Key, Value>>::size_type idx;
    idx = hasher_(key) % buckets_.size();
...
private:
  std::vector<std::unique_ptr<ThreadsafeBucketPair<Key, Value>>> buckets_;
  Hash hasher_;
...
public:
  ...
  bool contains(const Key& key) const {
    ...
    return (it != data_.end()) ? true : false;
  }
...
private:
   ...
   * @return std::list<std::pair<Key, T>>::iterator 
   */
  typename std::list<Key>::iterator find_entry_for(const Key& key) {
    auto it = data_.begin();
  ...
  typename std::list<Key>::const_iterator find_entry_for(const Key& key) const {
    auto it = data_.cbegin();
...
class ThreadsafeSet {
public:
  ThreadsafeSet(const unsigned int number_buckets = 19, const Hash& hasher=Hash())
  // : buckets_(number_buckets, std::make_unique<ThreadsafeBucketPair<Key,Value>>()),
  : hasher_(hasher) { 
    for(int i = 0; i < number_buckets; ++i)
      buckets_.push_back(
        std::unique_ptr<ThreadsafeBucket<Key>>(
          new ThreadsafeBucket<Key>()
        )
      );
  }
...
private:
  const ThreadsafeBucket<Key>& get_bucket(const Key& key) const {
    typename std::vector<ThreadsafeBucket<Key>>::size_type idx;
    idx = hasher_(key) % buckets_.size();

    return *buckets_[idx];
  }

  ThreadsafeBucket<Key>& get_bucket(const Key& key) {
    typename std::vector<ThreadsafeBucket<Key>>::size_type idx;
    idx = hasher_(key) % buckets_.size();
...
private:
  std::vector<std::unique_ptr<ThreadsafeBucket<Key>>> buckets_;
  Hash hasher_;

}; 


#endif // ASYNCHRONOUS_SERVER_THREADSAFE_DATA_STRUCTURES_HPP
